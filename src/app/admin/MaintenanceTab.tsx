...
function getDemoTickets(): Ticket[] {
  const d = demoDates();
  return [
    { id: "demo_1", title: "HVAC not cooling — Suite 301", description: "Tenant reports unit blowing warm air. Thermostat set to 68°F but room holds at 78°F.", building: "The Executive", unit: "Suite 301", category: "HVAC", priority: 1, status: "in_progress", assignedTo: "Mike D.", reportedBy: "Alex Harrison", estimatedCost: 650, actualCost: 0, estimatedHours: 4, scheduledDate: d.today, completedDate: null, notes: "[Apr 17 — Mike D.] Compressor cycling on and off. Ordered capacitor. Parts arrive tomorrow.", source: "admin", createdAt: new Date(Date.now() - 3600000 * 6).toISOString() },
    ...
    <div className="bg-[rgba(0,0,0,0.4)] border border-[rgba(250,204,21,0.2)] rounded-2xl p-5 mb-6 space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        ...
        <div>
          ...
          <div className="flex gap-1.5 items-center"><input value={form.reportedBy || ""} onChange={e => set("reportedBy", e.target.value)} placeholder={currentUserName || "Alex Harrison"} className={FIELD + " flex-1"} /><MicButton onResult={(t) => set("reportedBy", (form.reportedBy || "") + (form.reportedBy ? " " : "") + t)} /></div>
        </div>
        <div>
